...
In production DC designs at the scale of LinkedIn or AWS, MLAG (or equivalent vendor implementations — Arista uses "MLAG," Cisco NX-OS uses "vPC," Juniper uses "MC-LAG") is common for any server that requires full-bandwidth dual-homing. For servers where the bandwidth of one link is sufficient and switch-level redundancy is provided by the spine-leaf architecture above, active-passive bonding is simpler and sufficient.
